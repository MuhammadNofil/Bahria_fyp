const DrwaingService = require("../../Services/Drwaing/drawing.service");
const responses = require("../../Responses/response");
const create = async (req, res) => {
  try {
    const answer = await DrwaingService.create(req.body);
    if (!answer) {
...
const update = async (req, res) => {
  try {
    const answer = await DrwaingService.update(req.body, req.params);
    if (!answer) {
      res.send(responses.genericResponse(401, false, responses.FAILED));
      return;
    }
...
const getbyid = async (req, res) => {
  try {
    const answer = await DrwaingService.getbyid(req.params);
    if (!answer) {
      res.send(responses.genericResponse(401, false, responses.FAILED));
      return;
    }
...
const getall = async (req, res) => {
  try {
    const answer = await DrwaingService.getall();
    if (!answer) {
      res.send(responses.genericResponse(401, false, responses.FAILED));
      return;
    }
...
const Delete = async (req, res) => {
  try {
    const answer = await DrwaingService.Delete(req.params);
    if (!answer) {
      res.send(responses.genericResponse(401, false, responses.FAILED));
      return;
    }
